function strOfGivenLength(arr, strLength = 5) {
  let result = [];
  for (i = 0; i < arr.length; i++) {
    if (arr[i].length <= strLength) {
      result.push(arr[i])
    }
  }
  if (result.length !== 0) {
    return result
  }
  return false
}

console.log(strOfGivenLength(['test', 'delegation', 'forward', 'bliss', 'hop'], 4));
console.log(strOfGivenLength(['haste', 'hazelnut', 'hat', 'hacker', 'hiking']));

//Write a function that accepts an array of strings and an optional number (default value is 5). This function should return a new array containing strings with length <= the given number